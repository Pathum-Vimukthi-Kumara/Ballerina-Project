...
// Helper to set CORS headers on responses
...
service /api/org on mainListener {
    ...
    resource function get events/[int organization_id](http:Caller caller, http:Request req) returns error? {
        error? vErr = validateAuth(req, caller);
        if vErr is error { return; }
        OrgEvent[]|error events = getOrgEvents(organization_id);
        if events is OrgEvent[] {
            return caller->respond(events);
        } else {
            http:Response r = new;
            setCORSHeaders(r);
            r.statusCode = http:STATUS_BAD_REQUEST;
            r.setJsonPayload({"error": (<error>events).message()});
            return caller->respond(r);
        }
    }
...
service /api/admin on mainListener {
    // Approve organization endpoint (stub)
    resource function post organizations/[int id]/approve(http:Caller caller, http:Request req) returns error? {
        http:Response r = new;
        setCORSHeaders(r);
        // TODO: Implement actual approval logic here
        r.setJsonPayload({ message: "Organization approved (stub)" });
        return caller->respond(r);
    }
    // CORS preflight handler
    resource function options [string... path](http:Caller caller, http:Request req) returns error? {
    ...
    resource function patch users/[int id]/status(UserStatusUpdate body, http:Caller caller, http:Request req) returns error? {
        ...
        if res is string {
            r.statusCode = http:STATUS_OK;
            r.setJsonPayload({message: res});
            return caller->respond(r);
        }
        r.statusCode = http:STATUS_NOT_FOUND;
    ...
    resource function post volunteers/[int volunteer_id]/performance_badges(http:Caller caller, http:Request req) returns error? {
        ...
        if ap is error {
            ...
            setCORSHeaders(r);
            r.statusCode = http:STATUS_INTERNAL_SERVER_ERROR;
            r.setJsonPayload({"error": ap.message()});
            return caller->respond(r);
        }

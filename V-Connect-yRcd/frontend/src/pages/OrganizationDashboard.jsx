...
const OrganizationDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const eventsResponse = await fetch('http://localhost:9000/api/org/profile', {
          headers: {
        ...
        const orgEvents = await fetch(`http://localhost:9000/api/org/events/${profileData.id}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        for (const event of eventsData) {
          const applicationsResponse = await fetch(`http://localhost:9000/api/org/events/${event.event_id}/applications`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
  ...
  const handleApproveVolunteer = async (volunteerId, eventId) => {
    try {
      ...
      const response = await fetch(`http://localhost:9000/api/org/events/${eventId}/applications/${application.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,

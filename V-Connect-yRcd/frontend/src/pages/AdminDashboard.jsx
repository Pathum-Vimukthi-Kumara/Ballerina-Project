...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        ...
        setOrganizations(usersData.filter(u => u.user_type === 'organization'));
        setStats({
          totalVolunteers: usersData.filter(u => u.user_type === 'volunteer').length,
          totalOrganizations: usersData.filter(u => u.user_type === 'organization').length,
          totalEvents: 0, // You can update this if you fetch events elsewhere
          pendingOrganizations: usersData.filter(u => u.user_type === 'organization' && !u.is_active).length
        });
        // Optionally fetch reports if needed, or remove if not used
  ...
  const handleApproveOrganization = async (orgId) => {
    try {
      ...
      const response = await fetch(`http://localhost:9000/api/admin/organizations/${orgId}/approve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleRejectOrganization = async (orgId) => {
    try {
      ...
      const response = await fetch(`http://localhost:9000/api/admin/organizations/${orgId}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleResolveReport = async (reportId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:9000/api/admin/reports/${reportId}/resolve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
                          ...
                          <tr key={org.user_id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex space-x-2">
                                ...
                                <button
                                  onClick={() => handleDeleteUser(org.user_id)}
